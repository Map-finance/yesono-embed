...
const RelatedMarkets: React.FC<RelatedMarketsProps> = ({ currentMarketId, marketTitle }) => {
  ...
  return (
    ...
    >
      <h3 className="text-lg font-semibold text-(--text-primary) mb-3">{t.market.relatedMarkets}</h3>
      {/* 市场列表 */}
      <div className="space-y-3">
        {isLoading ? (
          ...
          <p className="text-sm text-(--text-secondary) text-center py-4">{t.market.common.noData}</p>
        ) : (
          relatedEvents.map((event) => (
            <div
              key={event.id}
              onClick={() => handleMarketClick(event.slug)}
              className="flex items-start gap-3 p-2 rounded-lg hover:bg-(--bg-hover) cursor-pointer transition-colors"
            >
              <ProxyImage
