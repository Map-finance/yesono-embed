export default function TimeDivision({ time }: { time: Date }) {
    ...
    return (
        <div className="bg-(--bg-secondary) px-1 text-sm flex flex-col justify-center items-center border-(--border) whitespace-nowrap cursor-pointer hover:bg-(--bg-hover)">
            <div className="font-semibold">{weekday}</div>
            <div className="text-(--text-secondary)">{monthDay}</div>
        </div>
    )
}